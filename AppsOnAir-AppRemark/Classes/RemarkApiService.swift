    ...
    struct RemarkApiService {
        ...
        static func apiGetSignInURL(
            apiGetSignInPassData: NSDictionary = [:], completion: @escaping ([String: Any]) -> Void
        ) {
                            ...
                            as? [String: Any]
                        {
                            ...
                            if let httpResponse = response as? HTTPURLResponse {
                                if httpResponse.statusCode != 200 {
                                    Logger.logInternal(
                                        "\(failedGetURL) \(statusCode) \(httpResponse.statusCode)")
                                }
        ...
        static func apiUploadImage(
            image: UIImage, signUrl: String, completion: @escaping (Bool) -> Void
        ) {
            ...
            let task = URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    ...
                    Logger.logInternal(
                        isSuccess
                            ? imageUploadSuccess
                            : "\(errorUploadImage) \(statusCode) \(httpResponse.statusCode)")
        ...
        static func apiAddRemark(
            apiPassData: NSDictionary = [:], completion: @escaping ([String: Any]) -> Void
        ) {
                    ...
                    if data != nil {
                            ...
                            as? [String: Any]
                        {
                            ...
                            if let httpResponse = response as? HTTPURLResponse {
                                if httpResponse.statusCode != 200 {
                                    Logger.logInternal(
                                        "\(failedToLoad) \(statusCode) \(httpResponse.statusCode)")
                                }
                                completion(json as [String: Any])
                            }
                        }
