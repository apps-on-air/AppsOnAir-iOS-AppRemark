    ...
    class RemarkController: UIViewController {
        ...
        override func viewDidLoad() {
            ...
            self.view.backgroundColor =
                UIColor.init(hex: pageBackgroundColor) ?? sColorPrimaryThemeLight

            self.navView.addShadow()
            self.navView.backgroundColor =
                UIColor.init(hex: appBarBackgroundColor) ?? sColorPrimaryThemeLight
            ...
            self.btnNavClose.addTarget(
                self, action: #selector(self.btnClose(_:)), for: .touchUpInside)
            ...
            self.txtDescription.attributedText = NSAttributedString(
                string: txtDescriptionHintText ?? descriptionHintText,
                attributes: [
                    NSAttributedString.Key.foregroundColor: UIColor.init(hex: hintColor)
                        ?? sColorTextLightGray
                ])
            ...
            clView?.register(
                UINib(nibName: "ImageViewCell", bundle: bundle),
                forCellWithReuseIdentifier: "ImageViewCell")
            clView?.register(
                UINib(nibName: "AddImageCVCell", bundle: bundle),
                forCellWithReuseIdentifier: "AddImageCVCell")
            ...
            btnSubmit.backgroundColor =
                UIColor.init(hex: buttonBackgroundColor) ?? sColorPrimaryThemeDark
            btnSubmit.setTitleColor(
                UIColor.init(hex: buttonTextColor) ?? sColorTextWhite, for: .normal)
        ...
        @objc func failureToast(isNetworkError: Bool = false) {
            self.showToast(message: isNetworkError ? errorNetwork : errorSomethingWentWrong) {
                success in
                if success {
                    self.hideLoader()
                }
        ...
        @objc func dismissController() {
            NotificationCenter.default.post(
                name: NSNotification.Name("visibilityChanges"), object: nil,
        ...
        @objc func btnSubmit(_ sender: UIButton) {
            ...
            if (AppRemarkService.shared.appsOnAirCore.isNetworkConnected ?? false)
                && !(appsOnAirAppId.isEmpty)
            {
                ...
                for index in 0..<(selectedImage?.count ?? 0) {
                    ...
                    let fullFileName =
                        "\((Bundle.main.appName ?? "").isEmpty ? "" : "\(Bundle.main.appName ?? "")_")\(epochTimeInMilliseconds)"
                    let fileType = "image/\(selectedImage?[index].getFileType() ?? "")"

                    // convert image bytes into MB for BackEnd
                    let imageSizeInBytes = Double(
                        self.selectedImage?[index].getImageSize()?.count ?? 0)
                    ...
                    //Api of getSignIn
                    RemarkApiService.apiGetSignInURL(apiGetSignInPassData: imageData) {
                        signInData in
                        guard
                            ...
                            let feedBackReponse = [
                                "status": APIStatus.failure.rawValue,
                                "message": signInData["message"] as? String
                        ...
                        RemarkApiService.apiUploadImage(
                            image: self.selectedImage?[index] ?? UIImage(), signUrl: signInUploadUrl
                        ) { value in
                            if value {
                                feedBackScreenShotData.append([
                                    "key": signInURL, "fileType": fileType, "size": imageSize,
                                ])
                                dispatchGroup.leave()
                            } else {
                                let feedBackReponse = [
                ...
                dispatchGroup.notify(queue: .main) {
                    ...
                    AppRemarkService.shared.appsOnAirCore.getDeviceInfo(additionalInfo: [
                        "appRemarkVersion": self.getPodVersion(),
                        "appLinkVersion": self.getPodVersion(isAppLink: true),
                        "appSyncVersion": self.getPodVersion(isAppSync: true),
                    ]) { deviceInfo in
                        var deviceDetails = (deviceInfo["deviceInfo"] as? [String: Any]) ?? [:]
                        let appPermission: [String: String] =
                            AdditionalDeviceInfo.permissionsInfo() as [String: String]
                        deviceDetails.merge([
                    ...
                    dispatchGroup.notify(queue: .main) {
                        ...
                        let remarkType: RemarkType =
                            (self.dropDown.text == self.ticketType.first) ? .suggestion : .bug
                        let userDescription = self.txtDescription.text.trimmingCharacters(
                            in: .whitespacesAndNewlines)
                        let description =
                            ([self.txtDescriptionHintText, self.descriptionHintText].contains(
                                userDescription) || userDescription.isEmpty) ? "" : userDescription
                        ...
                        let appId: NSDictionary = [
                            "appId": AppRemarkService.shared.appsOnAirCore.appId
                        ]
                        ...
                        RemarkApiService.apiAddRemark(apiPassData: apiPassingData) { feedBackInfo in
                            let message =
                                (feedBackInfo["message"] as? String)?.trimmingCharacters(
                                    in: .whitespacesAndNewlines)
                                ?? errorSomethingWentWrong
                            let isSuccess =
                                feedBackInfo["error"] == nil
                                && feedBackInfo["status"] as? String == APIStatus.success.rawValue
                                && !message.isEmpty
                            let status =
                                isSuccess ? APIStatus.success.rawValue : APIStatus.failure.rawValue
                            ...
                            func updateResponse() {
                                AppRemarkService.shared.remarkResponse = [
    ...
    extension RemarkController: UICollectionViewDelegate, UICollectionViewDataSource,
        UICollectionViewDelegateFlowLayout
    {
            ...
            -> UICollectionViewCell
        {
            ...
            guard indexPath.row < (self.selectedImage?.count ?? 0) else {
                let cell =
                    collectionView.dequeueReusableCell(
                        withReuseIdentifier: "AddImageCVCell", for: indexPath) as? AddImageCVCell
            ...
            let cell =
                collectionView.dequeueReusableCell(
                    withReuseIdentifier: "ImageViewCell", for: indexPath) as? ImageViewCell
            ...
            cell?.bgImageView.addShadows(
                shadowColor: sColorGray.cgColor, shadowOffset: CGSize(width: 5, height: 3),
                shadowOpacity: 0.9, shadowRadius: 6, cornerRadiusIs: 6)

            cell?.btnRemove.tag = inx
            cell?.btnRemove.addTarget(
                self, action: #selector(removeSnapshot(_:)), for: .touchUpInside)
