    ...
    import CoreMotion
    #if SWIFT_PACKAGE
    import Toast        // SPM module name
    #else
    import Toast_Swift  // CocoaPods module name
    #endif
    import AVKit
    import Photos
    ...
    extension UIViewController {
        ...
        static let classInit: Void = {
            ...
            let swizzledSelectorViewDidDisappear = #selector(
                UIViewController.swizzled_viewDidDisappear(_:))
            method_exchangeImplementations(originalMethod, swizzledMethod)

            guard
                let originalMethod = class_getInstanceMethod(
                    UIViewController.self, originalSelectorViewDidDisappear),
                let swizzledMethod = class_getInstanceMethod(
                    UIViewController.self, swizzledSelectorViewDidDisappear)
            else {
                return
            }
            method_exchangeImplementations(originalMethod, swizzledMethod)
        ...
        @objc func swizzled_viewDidLoad() {
            ...
            if let name = NSStringFromClass(type(of: self)).components(separatedBy: ".").last {
                ...
                if name == "RemarkController" || name == "_UIImagePickerPlaceholderViewController"
                    || name == "PUPhotoPickerHostViewController"
        ...
        @objc private func swizzled_viewDidDisappear(_ animated: Bool) {
            if let name = NSStringFromClass(type(of: self)).components(separatedBy: ".").last {
                ...
                if (name == "ZLEditImageViewController" || name == "RemarkController")
                    && isBeingDismissed
                {
        ...
        func handleShakeGesture() {
            ...
            ZLEditImageViewController.showEditImageVC(
                parentVC: self, image: screenshot ?? UIImage()
            ) { image, Editmodel in
                NotificationCenter.default.post(
                    name: NSNotification.Name("visibilityChanges"), object: nil,
                    userInfo: ["isPresented": true])
        ...
        func showToast(
            message: String, duration: Double = 2.0, completion: ToastCompletionHandler? = nil
        ) {
        ...
        func selectImagePopup(_ title: String? = chooseImageSource) {
            let alert = UIAlertController(
                title: nil, message: title, preferredStyle: UIAlertController.Style.actionSheet)

            if UIDevice.current.userInterfaceIdiom == .pad {
                if let popoverController = alert.popoverPresentationController {
                    popoverController.sourceView = self.view  // Change this to your actual view
                    popoverController.sourceRect = CGRect(
                        x: self.view.bounds.midX, y: self.view.bounds.height, width: 0, height: 0)
                    popoverController.permittedArrowDirections = []  // No arrow direction
                }
            }

            alert.addAction(
                UIAlertAction(title: gallery, style: .default) { (result: UIAlertAction) -> Void in
                ...
                if Bundle.main.photoPermission ?? false {
                    PHPhotoLibrary.requestAuthorization { status in
                        if status == .denied {
                            DispatchQueue.main.async {
                                let alert = UIAlertController(
                                    title: remark, message: errorGalleryPermission,
                            ...
                            if UIImagePickerController.isSourceTypeAvailable(.photoLibrary) {
                                DispatchQueue.main.async {
                                    ...
                                    imagePicker.delegate =
                                        (self
                                            as? UIImagePickerControllerDelegate
        ...
        public func showRemarkScreen(screenshot: UIImage? = nil) {
            ...
            let Vc =
                storyboard.instantiateViewController(withIdentifier: "RemarkController")
                as? RemarkController
    ...
    extension UIView {
        ...
        func addShadow() {
            ...
            layer.shadowPath =
                UIBezierPath(
                    rect: CGRect(
                        x: 0, y: bounds.maxY - layer.shadowRadius,
                        width: screenWidth,
    ...
    extension UIDevice {
        ...
        var modelIdentifier: String {
                ...
                if let simDeviceName = ProcessInfo.processInfo.environment[
